          ...
          <div className="px-8 py-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="relative w-full max-w-md">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 bg-[#f3f4f6] border border-[#e5e7eb] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#ef4444] focus:border-transparent transition-all"
              />
            </div>
          </div>
        </div>
          ...
          {classesLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredClasses.map(classe => (
                <div
                  key={classe.id}
                  className="group relative bg-white border-2 border-[#e5e7eb] rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 p-6 flex flex-col"
                >
                  {/* Bloc Red Circle */}
                  <div className="absolute top-6 right-6 z-10">
                    <div className="w-10 h-10 rounded-full bg-[#ef4444] flex items-center justify-center text-white font-bold text-lg shadow-lg border-4 border-white">
                      {classe.bloc}
                    </div>
                  </div>
                  {/* Class Name & Subject */}
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-sm text-[#374151] font-medium truncate">Vous</span>
                  </div>

                  {/* Students & Progress */}
                  <div className="flex items-center gap-3 mb-4">
                    <Users className="w-4 h-4 text-[#ef4444]" />
                    <span className="text-sm text-[#374151]">{classe.currentStudents} / {classe.maxStudents} étudiants</span>
                  </div>
                  ...
                  <div className="flex gap-3 mt-auto">
                    <Link
                      ...
                      className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-[#ef4444] text-white rounded-xl font-semibold hover:bg-[#dc2626] transition-colors shadow-md"
                    >
                    ...
                    <Link
                      ...
                      className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-black text-white rounded-xl font-semibold hover:bg-[#374151] transition-colors shadow-md"
                    >
